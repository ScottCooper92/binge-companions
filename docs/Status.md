...
| LIBRARY | `com.binge.integration.LIBRARY` | **Draft** | The v1 messages and `LibraryService` are in `contracts/`: availability, a play hand-off, watch state both ways, continue watching, each behind its own capability. The decisions are recorded in `Architecture.md`. The SDK support is #20, and Jellyfin is the first companion, so nothing has served it yet. |
